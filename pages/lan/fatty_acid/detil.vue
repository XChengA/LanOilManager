<template>
  <div class="container">
    <div class="view" v-if="state.data">
      <div class="left">
        <div class="text_one">{{ state.data.cn_name }}</div>
        <div class="text_two">{{ state.data.en_name }}</div>
        <div class="text_three">来源：<span v-if="state.data?.source" v-html="state.data.source"></span></div>
...
definePageMeta({
  pageTransition: config.getPageTransition("lan-fatty_acid-detil"),
});

const seoConfig = config.getPageSeoConfig("lan-fatty_acid-detil");
...
onMounted(() => {
  ...
  config.FATTY_ACID_LIST.forEach((i) => {
    if (i.id == id) {
      state.data = i.detil;
    }
  });
